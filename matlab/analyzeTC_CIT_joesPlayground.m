...
%%%%%%%%
  ...
  pos = find(Jn.req > 0.1 & ... % anything > 0.1 radians AND...
             (Jn.req - Jn.err0) < (2*pi - .1) | ... % not running into far hard stop
             (Jn.req > 0 & ~mv1)); % OR any positive subsequent moves
  neg = find(Jn.req < 0 & ... % anything negative
...
% $$$                  'Startpoint', 0, 'Weights',ones(size(neg))/(1e-3)^2);
  ...
  posslope = fit_slope_lin_lsq(Jn.req(pos), Jn.err0(pos), 1e-3);
  negslope = fit_slope_lin_lsq(Jn.req(neg), Jn.err0(neg), 1e-3);
