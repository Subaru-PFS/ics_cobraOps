...
if ~exist('ImgDir','var'), ImgDir = './';end; %Must end with /
data.ImgDir = ImgDir;

dataName = ['s2cntrs_' char(regexp(pwd,'(\d+_)*','match'))];
...
% Module ID for retrieving data from xml:
mID = 1; 
...
for pid=data.activeCobras
    data.centers(c) = getARMval(CobraConfig,pid, mID,'Global_base_pos_x')+getARMval(CobraConfig,pid, mID, 'Global_base_pos_y')*i;
    data.Rptrl(c) = 80;
    c=c+1;
end
