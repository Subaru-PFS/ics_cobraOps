function output=getS1CentersFromMsim(data,debugplots)
...
for ii=data.activeCobras
    ...
    lev1.R = Rtemp;
    % Cat theta with itself to encompass > 2*pi range. 
    ...
    resid0 = abs(data.(fldID).CCDpos - cfit0.c) - cfit0.R;
    resid2 = abs(data.(fldID).CCDpos - cfit2.c) - cfit2.R;
...
%         hold on
%         plot(data.(fldID).CCDpos-data.origin,'rx');
%         plot(data.(fldID).CCDpos,'bo');
...
%         figure(figID)
%         hold on
%         plot(unwrap(angle(data.(fldID).CCDpos-cfit1.c)),resid1,'r')
%         plot(unwrap(angle(data.(fldID).CCDpos(ok)-cfit2.c)),resid2,'b')
%         hold off
%         hgsave(strcat(data.CntrImgsDir,data.Name,'_',num2str(figID),'.fig'));
        catch err
            disp(err)
