...
function par=mtit(varargin)
  ...
  vin=varargin(vl);
  [offset,vout]=get_offset(vin{:});

  % find surrounding box
  axis_handle=findall(figh,'type','axes');
  if isempty(axis_handle)
    disp('mtit> no axis');
    return;
  end
  ou=get(axis_handle,'units');
  set(axis_handle,'units',defunit);
  ap=get(axis_handle,'position');
  if iscell(ap)
    ap=cell2mat(get(axis_handle,'position'));
  end
  ...
  return;
%--------------------------------------------------------------------------------
function [offset,vout]=get_offset(varargin)

% search for pairs <.offset>/<value>
  offset=[];
