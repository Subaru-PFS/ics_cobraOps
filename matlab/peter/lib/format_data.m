...
% author: peter mao, 20 Feb 2006
  ...
  if ~exist('precision','var')
    ...
    if ~isnumeric(precision)
      disp(['USAGE: outstring = format_data(datum, sigma, [precision],' ...
            ' [output_format], [tex]']);
      disp('PRECISION argument should be numeric');
      return;
    end
  end
  ...
  % does the datum or sigma increment in its log interval? (.99 -> 1.00)
